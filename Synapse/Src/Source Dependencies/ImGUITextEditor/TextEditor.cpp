...
//Disable assertations in TextEditor
#undef assert
#define assert(x)
...
const TextEditor::Palette & TextEditor::GetDarkPalette()
{
	const static Palette p = { {
		...
		0xFF00C6FF,	// Number
		0xFF3A8CFF,	// String
		0xFF3A8CFF, // Char literal
		0xffffffff, // Punctuation
		0xff408080,	// Preprocessor
		0xFFC8C8C8, // Identifier
		0xFFFFFFFF, // Known identifier
		0xFFFFFFFF, // Preproc identifier
		0xFF666666, // Comment (single line)
		0xFF666666, // Comment (multi line)
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::Lua()
{
	...
	if (!inited)
	{
		...
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>(R"(\[(=*)\[(.*?)\]\1\])", PaletteIndex::String));
		...
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>(R"([!#$%&’()*+,-./:;<=>?@[\]^_‘{|}~])", PaletteIndex::Punctuation));
