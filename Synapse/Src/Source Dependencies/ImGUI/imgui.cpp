...
FILE* ImFileOpenWChar(const wchar_t* filename, const wchar_t* mode)
{
#if defined(_WIN32) && !defined(__CYGWIN__) && !defined(__GNUC__)
	return _wfopen((wchar_t*) filename, (wchar_t*) mode);
#else
	return fopen(filename, mode);
#endif
}
...
void* ImFileLoadToMemoryWChar(const wchar_t* filename, const wchar_t* file_open_mode, size_t* out_file_size, int padding_bytes)
{
	IM_ASSERT(filename && file_open_mode);
	if (out_file_size)
		*out_file_size = 0;
	...
	FILE* f;
	if ((f = ImFileOpenWChar(filename, file_open_mode)) == NULL)
		return NULL;
...
void ImGui::LoadIniSettingsFromDisk(const wchar_t* ini_filename)
{
    size_t file_data_size = 0;
    char* file_data = (char*) ImFileLoadToMemoryWChar(ini_filename, L"rb", &file_data_size);
    if (!file_data)
        return;
...
void ImGui::SaveIniSettingsToDisk(const wchar_t* ini_filename)
{
    ImGuiContext& g = *GImGui;
    g.SettingsDirtyTimer = 0.0f;
    if (!ini_filename)
    ...
    FILE* f = ImFileOpenWChar(ini_filename, L"wt");
    if (!f)
        return;
